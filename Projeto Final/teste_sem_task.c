...
void Move_motor_x()
{
	...
	if(passos_x != 0) MOTOR_X_TASK_SLEEP = ((tempo_execucao*1E6)/abs(passos_x));
	//rt_task_set_periodic(NULL, TM_NOW, MOTOR_X_TASK_SLEEP);
	printf("Wait period motor x %d\n", MOTOR_X_TASK_SLEEP);
...
{
	int passo_atual_y;
	if(passos_y != 0) MOTOR_Y_TASK_SLEEP = ((tempo_execucao*1E6)/abs(passos_y));
	//rt_task_set_periodic(NULL, TM_NOW, MOTOR_Y_TASK_SLEEP);
	printf("Wait period motor y %d\n", MOTOR_Y_TASK_SLEEP);
...
void Calcula_trajeto(float atual_x, float atual_y, float final_x, float final_y)
{
	...
	if (passos_x != 0) Move_motor_x();
	if (passos_y != 0) Move_motor_y();
